...
class AECat(RuleModule):
    def __init__(self, rule_emb_size, n_concepts, n_tasks, n_rules):
        """
            ...
            and irrelevance. Therefore, they are mutually exclusive. This time using an autoencoder for rule changability
        """
        ...
        self.rule_encoder = torch.nn.Sequential(
            torch.nn.Linear(3 * self.n_concepts, self.rule_emb_size),
            torch.nn.LeakyReLU(),
            torch.nn.Linear(self.rule_emb_size, self.rule_emb_size),
        )

    def forward(self, rule_logits):
        # rule_logits shape: (batch, n_concepts * 3)
        latent = self.rule_encoder(rule_logits)
        reconstructed_logits = self.rule_decoder(latent)
...
class MNISTModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.info["ae_loss"].append(ae_loss.item())
